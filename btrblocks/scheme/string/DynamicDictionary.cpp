...
double DynamicDictionary::expectedCompressionRatio(StringStats& stats, u8) {
  ...
  if (stats.total_unique_length >= cfg.dict_fsst_input_size_threshold) {  // Threshold
...
u32 DynamicDictionary::compress(const btrblocks::StringArrayViewer src,
  ...
  col_struct.use_fsst =
      (cfg.dict_allow_fsst && (stats.total_unique_length >= cfg.dict_fsst_input_size_threshold)) ||
      cfg.dict_force_fsst;
  ...
  if (col_struct.use_fsst) {
    ...
    IntegerSchemePicker::compress(uncompressed_lengths.data(), nullptr, write_ptr,
                                  uncompressed_lengths.size(), cfg.fsst_codes_max_cascade_depth,
                                  used_space, col_struct.lengths_scheme,
                                  static_cast<u8>(autoScheme()));
  ...
  // Codes
  {
    ...
    IntegerSchemePicker::compress(codes.data(), nullptr, write_ptr, codes.size(),
                                  cfg.fsst_codes_max_cascade_depth, used_space,
...
bool DynamicDictionary::isUsable(StringStats& stats) {
  ...
  for (const auto& scheme : SchemePool::available_schemes->string_schemes) {
