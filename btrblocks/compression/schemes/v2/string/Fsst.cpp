...
u32 Fsst::compress(const btrblocks::StringArrayViewer src,
  ...
  die_if(fsst_export(encoder, write_ptr) > 0) auto fsst_table_used_space = FSST_MAXHEADER;
  ...
  // TODO Think about how to handle the cascading_level properly
  u32 used_space;
  IntegerSchemePicker::compress(reinterpret_cast<const INTEGER*>(src.slots_ptr), nullptr, write_ptr,
                                stats.tuple_count + 1, 2, used_space, col_struct.offsets_scheme);
...
bool Fsst::isUsable(StringStats& stats) {
  ...
  return stats.total_length > FSST_THRESHOLD;
}
