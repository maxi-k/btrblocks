...
#include <iomanip>
// -------------------------------------------------------------------------------------
DEFINE_uint32(MaxExponent_siginifcant_digit_bits_limit, 64, "");
// -------------------------------------------------------------------------------------
using namespace std;
...
u32 MaxExponent::compress(const DOUBLE* src,
  ...
  for (u32 row_i = 0; row_i < stats.tuple_count; row_i++) {
    ...
    for (e = 0; e <= max_exponent; e++) {
      ...
      if (if_converted_back == current_double &&
          ((std::floor(std::log2(sd)) + 1) <= FLAGS_MaxExponent_siginifcant_digit_bits_limit)) {
        convertable = true;
        break;
      }
    }
  ...
  if (sd_v.size()) {
    ...
    IntegerSchemePicker::compress(sd_v.data(), nullptr, write_ptr, sd_v.size(),
                                  ...
                                  AUTO_SCHEME, "significant digits");
