...
for filename in glob.iglob(base_dir + '/**/*.double', recursive=True):
    files.append({"values": filename, "bitmap": filename.replace('.double', '.bitmap')})

stats = []
for column in files:
    print("reading " + column['values'])
    try:
        data = np.fromfile(column['values'], dtype=np.float64)
